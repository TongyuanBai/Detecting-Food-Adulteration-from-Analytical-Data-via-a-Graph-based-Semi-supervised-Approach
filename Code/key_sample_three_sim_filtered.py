...
file_path = Path('example_filtered.csv')
...
def process_distance_matrix(dist_matrix, dist_name):
    ...
    Process a given distance matrix:
      ...
      5) Select key nodes by degree >= 95th percentile
    ...
    for community, nodes in communities.items():
        ...
        # Current: select nodes whose degree is >= 95th percentile (degree_threshold).
        # Alternative: to control the proportion of labeled samples (e.g., 10%, 20%, 50%),
        #              replace the rule below with a top-k% cutoff:
        #              top_k_count = max(1, int(len(sorted_nodes) * 0.10))  # 10% example
        #              filtered_nodes = sorted_nodes[:top_k_count]
        #
        # This way, you can explicitly set how many nodes in each community are treated as "known labels".
    ...
    output_file_path = output_dir / f"Groundnut_mst_filtered_{dist_name}.xlsx"
